...
import { emptyExamAnswer, hasUngradedOpenPart, isExamComplete, isExamSelfGraded, scoreExamTask } from '../../types'
import { ExamResultView } from './ExamResultView'
import { ExamSelect } from './ExamSelect'
import { ExamSidebar } from './ExamSidebar'
import { ExamTaskView } from './ExamTaskView'
...
function freshSession(exam: GraphExam): Session {
  return {
    exam,
    index: 0,
    answers: exam.tasks.map(emptyExamAnswer),
...
type Props = {
  /** Back to the graph landing page. */
  onLeave: () => void
}

/** Root of the exam trainer: exam list, then one exercise at a time with a sidebar. */
export function GraphExamApp({ onLeave }: Props) {
  ...
  const totalScored = useMemo(() => {
    ...
    return session.exam.tasks.reduce(
      (sum, task, i) => sum + (session.confirmed[i] ? scoreExamTask(task, session.answers[i]) : 0),
      0,
    )
  }, [session])
  ...
  const setAnswer = (next: ExamAnswer) => {
    const updatedAnswers = [...answers]
    updatedAnswers[index] = next
    // A mixed exercise (auto-graded + open parts) becomes done the moment every open
    // part has points, without waiting for the manual toggle.
    const updatedDone = [...done]
    if (confirmed[index] && isExamSelfGraded(task) && !hasUngradedOpenPart(task, next)) {
      updatedDone[index] = true
    }
    update({ answers: updatedAnswers, done: updatedDone })
  }
  ...
  const confirm = () => {
    ...
    const updatedDone = [...done]
    if (!isExamSelfGraded(task)) updatedDone[index] = true
    ...
    <div className="flex min-h-screen">
      <ExamSidebar
        ...
        onSelectSubtask={(taskIndex, subtaskId) => {
          ...
          requestAnimationFrame(() => document.getElementById(subtaskId)?.scrollIntoView({ block: 'center' }))
        }}
      ...
      <main ref={main} className="h-screen flex-1 overflow-y-auto">
        <div className="mx-auto w-full max-w-4xl px-8 py-8">
          <p className="mb-4 text-[13px] text-gray-500 dark:text-gray-400">
            Aufgabe {index + 1} von {exam.tasks.length} · {exam.title}
          </p>

          <ExamTaskView key={task.id} task={task} answer={answer} evaluated={evaluated} onChange={setAnswer} />
          ...
          <div className="mt-8 flex flex-wrap items-center justify-between gap-3 border-t border-gray-200 pt-6 dark:border-gray-700">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <button
                type="button"
                disabled={evaluated || !isExamComplete(task, answer)}
                onClick={confirm}
                className={
                  evaluated
