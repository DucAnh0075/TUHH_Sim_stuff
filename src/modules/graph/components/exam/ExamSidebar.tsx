...
import { isExamStarted, scoreExamTask } from '../../types'
...
/**
 * The vertical exercise bar: every task of the exam with its state.
  ...
  return (
    <aside className="flex h-screen w-72 shrink-0 flex-col border-r border-gray-300 bg-gray-50 dark:border-gray-700 dark:bg-gray-900">
      <div className="border-b border-gray-300 px-4 py-4 dark:border-gray-700">
        ...
        <h1 className="mt-2 text-[16px] leading-tight font-bold text-gray-900 dark:text-gray-100">{exam.title}</h1>
        <p className="mt-1 text-[13px] text-gray-600 dark:text-gray-400">
          <strong className="text-gray-900 dark:text-gray-100">{formatPoints(totalScored)}</strong> / {exam.totalPoints}{' '}
          Punkte
        </p>
      ...
      <nav className="flex-1 overflow-y-auto px-2 py-2">
        <ol className="flex flex-col gap-1">
          {exam.tasks.map((task, i) => {
            ...
            const isDone = done[i]
            const started = isExamStarted(answers[i])
            ...
            // Sidebar subtasks: one entry per labelled part, deduped by group.
            const subtasks = groupParts(task.parts)
              .filter((run) => run[0].label || run[0].group)
              .map((run) => ({ id: run[0].id, label: run[0].group ?? run[0].label ?? '' }))
              ...
              <li key={task.id}>
                ...
                >
                  ...
                  <span className="shrink-0 text-[12px] font-bold whitespace-nowrap">
                    {confirmed[i] ? `${formatPoints(scoreExamTask(task, answers[i]))}/${task.points}` : `${task.points} P`}
                  </span>
                </button>
