...
export function Sidebar({
  ...
  return (
    <aside className="flex h-screen w-72 shrink-0 flex-col border-r border-gray-300 bg-gray-50">
      <div className="border-b border-gray-300 px-4 py-4">
        <button
          type="button"
          onClick={onLeave}
          className="cursor-pointer text-[12px] text-gray-500 underline underline-offset-2 hover:text-gray-800"
        >
          ← Klausur wechseln
        </button>
        <h1 className="mt-2 text-[16px] leading-tight font-bold text-gray-900">{exam.term}</h1>
        <p className="mt-1 text-[13px] text-gray-600">
          <strong className="text-gray-900">{formatPoints(totalScored)}</strong> / {exam.totalPoints} Punkte
        </p>
          ...
          {exam.tasks.map((task, i) => {
            ...
            return (
              <li key={task.id}>
                ...
                {task.subtasks && (
                  <ul className="mt-1 mb-1 ml-7 flex flex-col gap-0.5 border-l border-gray-300 pl-2">
                    {task.subtasks.map((subtask) => (
                      <li key={subtask.id}>
                        <button
                          type="button"
                          onClick={() => onSelectSubtask(i, subtask.id)}
                          className="w-full cursor-pointer rounded px-1.5 py-1 text-left text-[12px] leading-snug text-gray-600 hover:bg-gray-100 hover:text-gray-900"
                        >
                          {subtask.label}
                        </button>
                      </li>
      ...
      <div className="border-t border-gray-300 px-4 py-3">
        <button
          ...
          className="w-full cursor-pointer rounded-lg bg-gray-900 px-4 py-2 text-[14px] font-semibold text-white transition-colors hover:bg-gray-700"
        >
          Ergebnis anzeigen
        </button>
      </div>
