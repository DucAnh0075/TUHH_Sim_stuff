...
export function QuizApp() {
  ...
  if (finished) {
    ...
    return (
      <Shell>
        <div className="rounded-lg border-2 border-gray-300 bg-white px-8 py-10 text-center shadow-sm">
          <h2 className="text-[26px] font-bold text-gray-900">Result</h2>
          <p className="mt-4 text-5xl font-extrabold text-gray-900">
            {totalScored.toFixed(2)}
            <span className="text-2xl font-semibold text-gray-400"> / {totalMax.toFixed(2)}</span>
          </p>
          <p className="mt-2 text-gray-600">{pct}% of the achievable points</p>

          <ul className="mx-auto mt-8 flex max-w-md flex-col gap-2 text-left">
            {tasks.map((task, i) => (
              <li
                key={task.id}
                className="flex items-center justify-between gap-4 rounded border border-gray-200 px-3 py-2 text-[14px]"
              >
          ...
          <div className="mt-8 flex justify-center gap-3">
            <button
              ...
              className="rounded-lg border-2 border-gray-300 px-6 py-2.5 font-semibold text-gray-700 transition-colors hover:border-gray-500 hover:text-gray-900"
            >
              Back
            </button>
            <button
              type="button"
              onClick={restart}
              className="rounded-lg bg-gray-900 px-6 py-2.5 font-semibold text-white transition-colors hover:bg-gray-700"
            >
              Restart
            </button>
          </div>
  ...
  return (
    <Shell>
      <div className="mb-4 flex items-center justify-between text-[13px] text-gray-500">
        <span>
      ...
      <div className="mt-8 flex justify-between gap-3">
        <button
          ...
          disabled={index === 0}
          onClick={back}
          className="rounded-lg border-2 border-gray-300 px-6 py-2.5 font-semibold text-gray-700 transition-colors hover:border-gray-500 hover:text-gray-900 disabled:cursor-not-allowed disabled:opacity-40 disabled:hover:border-gray-300 disabled:hover:text-gray-700"
        >
          Back
        </button>
        ...
        <div className="flex gap-3">
          ...
          <button
            ...
            className={
              evaluated
                ? 'rounded-lg border-2 border-gray-300 px-6 py-2.5 font-semibold text-gray-400'
                : 'rounded-lg bg-gray-900 px-6 py-2.5 font-semibold text-white transition-colors hover:bg-gray-700 disabled:cursor-not-allowed disabled:opacity-40'
            }
          >
          ...
          <button
            ...
            className={
              evaluated
                ? 'rounded-lg bg-gray-900 px-6 py-2.5 font-semibold text-white transition-colors hover:bg-gray-700'
                : 'rounded-lg border-2 border-gray-300 px-6 py-2.5 font-semibold text-gray-700 transition-colors hover:border-gray-500 hover:text-gray-900'
            }
          >
